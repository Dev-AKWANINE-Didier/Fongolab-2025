@extends('admin.base')
@section("title","Creation de produit")
@section("content")
    <!-- CSS Animations -->
<style>
  @keyframes bounce {
    ...
    <article class="w-full flex items-center justify-center  min-h-screen px-4 bg-transparent">
      <section class="w-full article max-w-md bg-transparent rounded-xl shadow-lg p-6 sm:p-8 border-[#0c0c0d05] border animate-fade-in ">
          ...
          <header class="text-center mb-6">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-[#4c11bb] via-[#4c11bb] to-[#9A88F9] bg-clip-text">Ajouter un produit </h1>
          </header>

          <!-- Formulaire -->
          <form action="{{route('dashboard-products-store')}}" method="POST" class="space-y-6 text-black">
           @csrf
            <!-- Nom -->
            <div>
