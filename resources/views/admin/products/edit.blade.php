    ...
    <article class="w-full flex items-center justify-center  min-h-screen px-4 bg-transparent">
      <section class="w-full article max-w-md bg-transparent rounded-xl shadow-lg p-6 sm:p-8 border-[#0c0c0d05] border animate-fade-in ">
          ...
          <header class="text-center mb-6">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-[#4c11bb] via-[#4c11bb] to-[#9A88F9] bg-clip-text">Modifier le produit {{ $product->name }}</h1>
          </header>

          <!-- Formulaire -->
          <form action="{{route('dashboard-products-update',$product->id)}}" method="POST" class="space-y-6 text-black">
           @csrf
           @method("PUT")
            <!-- Nom -->
            <div>
            ...
            <div>
              ...
              <select name="category" id="category" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4c11bb] focus:outline-none focus:border-transparent transition duration-300">
            ...
            {{-- description --}}
               <div>
                 ...
                 <textarea name="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4c11bb] focus:outline-none focus:border-transparent transition duration-300" id="">{{ $product->description }}</textarea>
               @error('description')
                  <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
